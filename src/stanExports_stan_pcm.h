...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(177, 175, "end", "model_stan_pcm");
    return reader;
}
...
pcm_probs(const T0__& theta1,
    ...
    try {
        ...
        Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  unsummed(static_cast<Eigen::VectorXd::Index>((cols(beta1) + 1)));
        ...
        Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  probs(static_cast<Eigen::VectorXd::Index>((cols(beta1) + 1)));
        ...
        Eigen::Matrix<local_scalar_t__,Eigen::Dynamic,1>  probsx(static_cast<Eigen::VectorXd::Index>((cols(beta1) + 1)));
        (void) probsx;  // dummy to suppress unused var warning

        stan::math::initialize(probsx, DUMMY_VAR__);
        ...
        if (as_bool(logical_eq(revx,0))) {
            ...
            for (int p = 1; p <= 5; ++p) {
                ...
                stan::model::assign(probsx, 
                            stan::model::cons_list(stan::model::index_uni(p), stan::model::nil_index_list()), 
                            get_base1(probs,(6 - p),"probs",1), 
                            "assigning variable probsx");
            }
        }
...
typedef model_stan_pcm_namespace::model_stan_pcm stan_model;
